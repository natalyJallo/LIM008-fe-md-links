...
var _index = require("./md-links.js/index.js");

var _index2 = require("./module/module1/stats.js/index.js");
...
var comandLineFunction = function comandLineFunction(args, options) {
  if (args.length === 0) {
    ...
    if (args[0] === '--help') {
      ...
      (0, _index.mdLinks)(path, options).then(function (resp) {
        resp.forEach(function (values) {
          return console.log(" Path: ".concat(values.file, "\n Link: ").concat(values.href, "\n Text: ").concat(values.text.length > 50 ? "".concat(values.text.substring(0, 46), "...") : values.text));
        });
      }).catch(function (err) {
        return err;
      });
    ...
    if (args[1] === '--validate' || args[1] === '--v') {
      ...
      Promise.all([(0, _index2.totalstatsLinks)(path), (0, _index2.uniqueStatsLinks)(path)]).then(function (resp) {
    ...
    if ((args[1] === '--validate' || args[1] === '--v') && (args[2] === '--stats' || args[2] === '--s')) {
      Promise.all([(0, _index2.totalstatsLinks)(path), (0, _index2.uniqueStatsLinks)(path), (0, _index2.brokenStatsLinks)(path)]).then(function (resp) {
        return resp.forEach(function (values) {
          return console.log(values);
        });
      }).catch(function (err) {
